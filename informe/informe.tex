...
El primer bit $p$ es el que indica que la interrupción está presente en memoria.
...
Además como lo que queremos definir son puertas de interrupción, hay que definir como $1 1 0$ los 3 bits de $tipo$ $de$ $descriptor$.
...
Para la segunda funcionalidad se utiliza la función $sched$ alojada en el archivo $sched.c$ que detallaremos en el ejercicio 6. \\

\subsubsection{Handler de la interrupción 33 (Pausar Reanudar)}
...
Teniendo en cuenta que no se pasa por parametro cual es la tarea que llamo a la interrupción, la misma debe obtenerse a traves de la función tarea\_actual.
...
\begin{pseudocodigo}
  ...
  \IF {$eax = 111$}
    \STATE llamar a la función $game\_duplicar(nro\_jugador,fila,columna)$
  \ELSE
    \IF{$eax = 222$}
      \STATE llamar a la función $game\_migrar(col\_dst,fila\_dst,col\_src,fila\_src,nro\_jugador)$
    \ELSE
...
\subsubsection{Handler de la excepciones}

En este TP cuando se genera un error, se deduce que la tarea producente del mismo, lo genero intentando romper la seguridad del SO. Es por esto que se pasa directamente a elimiar la tarea del scheduler. Hay que tener en cuenta que una tarea tiene permitido pedir memoria, por lo que la unica excepción que no elimina a las tareas del scheduler es la $14$ ($Page$ $Fault$).

Veremos entonces, por un lado, la estrucutra creada para las excepciones que se encargan de eliminar la tarea actual del scheduler, y por otro el handler del $Page$ $Fault$.\\

\textbf{Estructura general para eliminar la tarea actual del scheduler.}\\
...
El scheduler es el encargado de ir pasandole el control del procesador de una tarea a la otra. Para hacer esto utiliza la interrupción de reloj como control del tiempo que una tarea lleva en ejecución. De esta manera cuando pasa una cierta cantidad de tiempo acumulado en un quantum le quitamos el control a una tarea y se lo pasamos a la siguiente. También se le quita el control a una tara si las mismas hacen una jugada correcta.
