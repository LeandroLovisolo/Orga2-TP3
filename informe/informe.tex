...
Los descriptores de las TSS son iguales que los descriptores de segmento, con la direrencia que en el campo $type$ hay que ponerle el valor de $9$ el cual le indica que es un descriptor de TSS, también tienen seteado en $0$ el bit $s$ para indicar que es un descriptor de sistema, esto es correcto ya que las TSS se consideran parte del sistema.

Por otro lado la base de los mismos se las define en la posición de memoria donde comienza la TSS de cada tarea, ya que ahi es donde se asumirá que están los datos correctos de la tarea a la cual se quiere pasar. Como límite se colocó $0x67$, el tamaño de la estructura de datos de una TSS.

Estas entradas de la GDT fueron definidas en la función $gdt_tareas_ii()$ que se puede encontrar en $gdt.c$ y son inicializadas desde $kernel.asm$.
...
Definimos las TSS de la taréa idle y tareas 1, 2, 3, 4 y 5 en el archivo $tss.c$.

\textbf{Rellenamos las TSS para cáda tarea de la manera siguiente:}
\begin{itemize}
 \item Definimos para cada tarea su ESP y EBP inicial para conformar su pila utilizando la dirección virtual previamente mapeada TASK\_STAK y sumándole $0x1000$ debido a que la pila trabaja decrementándose
 \item Asignamos a CR3 el valor correspondiente a la page directory asociada a la tarea.
 \item Definimos iomap con 0xFFFF para setear correctamente los usos de los puertos de entrada/salida y definimos los EFLAGS con 0x00000202 los cuales nos permitieron tener habilitadas las intrucciones.
 \item Definimos los registros segmentos de datos y códigos con el siguiente macro:
 
 $\#define\ SEGMENT\_SELECTOR(index,\ rpl)\ ((index << 3)\ |\ rpl)$
 
 El cuál se encarga de, dado un índice en la GTD y un RPL específico definir el segmento adecuado.
 \item Definimos para el caso de las tareas de privilegios de nivel 2 y 3 una pila de nivel 0 (la dirección fue provista por la cátedra) y un ss0 (registro de segmento) para la utilización de esta pila.

Para el caso de la tarea idle la cuál se ejecuta en nivel 0 no fue necesario declarar el segmento de datos para la pila.
\end{itemize}

\textbf{Ejemplo de definición de TSS con la tarea 1:}
